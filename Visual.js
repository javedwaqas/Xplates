Visual = function (canvas, visual, x, y, w, h, parent_visual, branch, start){
	this.canvas=canvas;
	this.visual=visual;
...
Visual.prototype.visualize = function(){
	...
	this.title_string = this.visual.title;
	this.title.attr({"x":(this.x+this.width/2), "y":(this.y+this.padding/2+this.title.getBBox().height/2), "font-size":30, "font-weight":"bold"});
...
//	******************** Adding Annotation *********************************
	...
	this.annotation_text.attr({"text-anchor":'start', "x":(this.x+this.padding+40), "y":(this.y-this.annotation_text.getBBox().height-14), "font-size":25});
	this.annotation_rect = this.canvas.rect((this.x+this.padding+38), (this.y-this.annotation_text.getBBox().height-12), this.annotation_text.getBBox().width+5, this.annotation_text.getBBox().height+5, 5).attr({"fill":"yellow", "fill-opacity":1, "stroke":"yellowgreen", "stroke-width":3});
	
	this.annotation.push(this.annotation_rect);
	this.annotation.push(this.annotation_text);
	this.annotation_sign = this.canvas.image("http://upload.wikimedia.org/wikipedia/commons/2/20/Text-x-generic.svg", this.x+this.padding+30, this.y+this.padding/4, 30, 30);	
...
Visual.prototype.addListeners = function (visual, canvas, node){
	var sx=0, sy=0;
	visual.glow;
	var isDrag = false;
...
Visual.prototype.unSelect = function(){
	this.isparentvisual = false;
	if (this.select_glow)
		this.select_glow.remove();
};

Visual.prototype.getBBox = function(){
	var dimension = {};
	dimension.x = this.x;
	dimension.y = this.y;
...
Visual.prototype.translateTo = function(newX, newY){
...
Visual.prototype.showAnnotation = function(){
...
Visual.prototype.addIt = function (wire){
...
Visual.prototype.addInputAnchorListeners = function (vis, anchor, wire){
...
Visual.prototype.updateInputAnchors = function(){
...
Visual.prototype.updateInputWires = function(){
	for (var i=0; i < this.wire_list.length; i++){
		this.wire_list[i].updatePosition();
	}
};
...
Visual.prototype.addOutputAnchor = function (data){
...
Visual.prototype.addSpecialOutputAnchor = function (data1, data2, label1, label2){
	var wire1, wire2;
...
Visual.prototype.removeWire = function(wire){
	var i = Xplate.findArrayIndex(this.o_wire_list, wire, 0);
	this.o_wire_list.splice(i, 1);
};

Visual.prototype.addOutputAnchorListeners = function (visual, node, child, data){
	node.mouseover(function (){
...
Visual.prototype.addSpecialOutputAnchorListeners = function (visual, node, clone_wire){
