...
Wordle.prototype.visualization = function (wordList, frequency){
	...
	this.MIN_UNIT_SIZE = 10;
	this.MAX_UNIT_SIZE = 40;
	...
	for (var ii = 0; ii < wordList.length; ii++) {
	    ...
	    //textColor = this.makeGradientColor({r:92, g:179, b:198}, {r:50, g:63, b:72}, 100*(freq - minFreq)/range).cssColor;
//		textColor = this.makeGradientColor({r:56, g:114, b:89}, {r:50, g:63, b:72}, 100*(freq - minFreq)/range).cssColor;
	    //var opacity = 0.5 + ((freq - minFreq) / range * 0.5);
	    var opacity = 1;
	    var temp_text = text;
//	    if (temp_text.length > 23)
//			temp_text = temp_text.substring(0, 20)+"\n"+temp_text.substring(20, temp_text.length);
		var text1 = paper.text(0,0,temp_text).attr({"text-anchor":'start',"fill":textColor,"font-size":size, "cursor":"default","font":paper.getFont("Museo"), "fill-opacity":opacity,"title":"Click to search: "+text});
	    ...
	    if (ii == 0){
	    	while(w>this.width || h>this.height){
	    		text1.remove();
	    		if (this.MIN_UNIT_SIZE<this.MAX_UNIT_SIZE)
	    			this.MAX_UNIT_SIZE = this.MAX_UNIT_SIZE-1;
	    		else{
	    			if (temp_text.length > 23)
						temp_text = temp_text.substring(0, 20)+"...";
	    		}
	    		if (this.MIN_UNIT_SIZE>10)
	    			this.MIN_UNIT_SIZE = this.MIN_UNIT_SIZE-1;
				this.RANGE_UNIT_SIZE = this.MAX_UNIT_SIZE - this.MIN_UNIT_SIZE;
				size = this.MIN_UNIT_SIZE + Math.round((freq - minFreq) / range * this.RANGE_UNIT_SIZE);
				
				text1 = paper.text(0,0,temp_text).attr({"text-anchor":'start',"fill":textColor,"font-size":size, "cursor":"default","font":paper.getFont("Museo"), "fill-opacity":opacity,"title":"Click to search: "+text});
				this.tagCloud.push(text1);
			    w = text1.getBBox().width + padding;
			    h = text1.getBBox().height + padding;
	    	}
	    				    	
	    	text1.attr({"x":this.cx-(w/2),"y":this.cy-(h/2)});
	    	// Add new boxes to the list
	    	var textBox = new BoundingBox(text1.getBBox().x-this.padding/2, text1.getBBox().y-this.padding/2, text1.getBBox().width+this.padding, text1.getBBox().height+this.padding, this.cx, this.cy);
	    	
	    	var dim = new BoundingBox(text1.getBBox().x-this.padding/4, text1.getBBox().y-this.padding/4, text1.getBBox().width+this.padding/2, text1.getBBox().height+this.padding/2, this.cx, this.cy);
//	    	var text_rect =this.canvas.rect(dim.x, dim.y, dim.width, dim.height, dim.height*0.1).attr({"fill":highlightColor, "stroke":null}).toBack();
//	    	this.tagCloud.push(text_rect);
			var cross = this.canvas.set();
			//this.cross_size = 10*opacity;
			this.cross_size = 5;
			var shift = -2;
			t = this;
//			cross_rect = this.canvas.rect((dim.x+dim.width-5-this.cross_size), (dim.y+2), this.cross_size, this.cross_size).attr({"fill":"red", "title": "click to remove", "cursor":"pointer", "stroke":null, "opacity":1});
			cross_rect = this.canvas.ellipse((dim.x+dim.width-shift-this.cross_size/2), (dim.y+shift+this.cross_size/2), this.cross_size, this.cross_size).attr({"fill":"lightgray", "title": "click to remove", "cursor":"pointer", "stroke":"#2f4f4f", "fill-opacity":1});
			cross.push(this.canvas.path("M"+(dim.x+dim.width-shift)+" "+(dim.y+shift)+"L"+(dim.x+dim.width-shift-this.cross_size)+" "+(dim.y+shift+this.cross_size)).attr({"stroke":"#2f4f4f", "title": "click to remove", "cursor":"pointer"}).toFront());
			cross.push(this.canvas.path("M"+(dim.x+dim.width-shift-this.cross_size)+" "+(dim.y+shift)+"L"+(dim.x+dim.width-shift)+" "+(dim.y+shift+this.cross_size)).attr({"stroke":"#2f4f4f", "title": "click to remove", "cursor":"pointer"}).toFront());
			this.tagCloud.push(cross_rect);
			this.tagCloud.push(cross);			
			cross.hide();
			cross_rect.hide();
//			this.addCrossListener(text_rect, cross, text, false);
			this.addWordListeners(text1, text, opacity, cross, cross_rect, textColor);
//			this.addCrossListener(text1, cross, text, cross_rect, text1, opacity, textColor, false);
//			this.addCrossListener(cross, cross, text, cross_rect, text1, opacity, textColor, true);
//			this.addCrossListener(cross_rect, cross, text, cross_rect, text1, opacity, textColor, true);
	    	
	    	this.updateBoxList(availableBoxes, textBox, boundingBox);
...
//			paper.rect(text1.getBBox().x, text1.getBBox().y, text1.getBBox().width, text1.getBBox().height, 5).attr({"fill":highlightColor, "stroke":null}).toBack();
	...
	//    		}
			...
			posy = posy + (padding/2);
	    		...
	    		// Add new boxes to the list
	    		textBox = new BoundingBox(text1.getBBox().x-this.padding/2, text1.getBBox().y-this.padding/2, text1.getBBox().width+this.padding, text1.getBBox().height+this.padding, this.cx, this.cy);
	    		
	    		dim = new BoundingBox(text1.getBBox().x-this.padding/4, text1.getBBox().y-this.padding/4, text1.getBBox().width+this.padding/2, text1.getBBox().height+this.padding/2, this.cx, this.cy);
//		    	text_rect =this.canvas.rect(dim.x, dim.y, dim.width, dim.height, dim.height*0.1).attr({"fill":highlightColor, "stroke":null}).toBack();
//		    	this.tagCloud.push(text_rect);
				cross = this.canvas.set();
				//this.cross_size = 10*opacity;
				this.cross_size = 5;
				shift = -2;
				t = this;
...
Wordle.prototype.visualization_old = function (wordList, frequency){
	var paper = this.canvas;
	var paper_w = this.width, paper_h = this.height;
	this.cx = this.x+this.width/2;//paper_w/2;
	this.cy = this.y+this.height/2;//paper_h/2;
